 ...
 */
export function validateModel(model, timeSeriesParams = {}) {
  const errors = [];
  ...
  const validNames = new Set([
    ...
    ...Object.keys(timeSeriesParams),  // Add time series param names
...
function extractReferences(equation) {
  // Handle non-string rates (e.g., constant numbers like rate: 1)
 ...
 * @param {boolean} options.strict - If true, throw on equation errors instead of returning 0
 * @param {object} options.timeSeriesParams - Time-varying parameters { paramName: [value0, value1, ...] }
 * @returns {object} - Results with time series for each stock
 */
export function simulate(model, options = {}) {
  const { steps = 100, dt = 1, strict = false, timeSeriesParams = {} } = options;

  // Validate first (pass timeSeriesParams so validator knows they'll be available)
  const validation = validateModel(model, timeSeriesParams);
  if (!validation.valid) {
  ...
  // Apply time-varying params at t=0
  applyTimeSeriesParams(state, timeSeriesParams, 0);
  ...
  for (let step = 0; step < steps; step++) {
    // Apply time-varying parameters for current step
    applyTimeSeriesParams(state, timeSeriesParams, step);

    // Calculate all flow rates at current state
    const flowRates = calculateFlows(model, state, { strict });
...
/**
 * Apply time-series parameter values for a given step.
 * This allows parameters to change over time (e.g., seasonal multipliers).
 *
 * @param {object} state - Current simulation state to modify
 * @param {object} timeSeriesParams - Map of param name to array of values
 * @param {number} step - Current simulation step
 */
function applyTimeSeriesParams(state, timeSeriesParams, step) {
  for (const [paramName, values] of Object.entries(timeSeriesParams)) {
    if (Array.isArray(values) && values.length > 0) {
      // Use the value at this step, or last value if beyond array length
      const index = Math.min(step, values.length - 1);
      state[paramName] = values[index];
    }
  }
}
 ...
 * @param {boolean} options.strict - If true, throw on equation errors
 */
function calculateFlows(model, state, options = {}) {
  const rates = {};
  ...
  for (const [flowName, flow] of Object.entries(model.flows)) {
    if (typeof flow.rate === 'number') {
      ...
      rates[flowName] = evaluateEquation(flow.rate, state, rates, options);
    } else {
      rates[flowName] = 0;
    }
 ...
 * @param {object} state - Current simulation state
 * @param {object} rates - Previously calculated flow rates
 * @param {object} options - Evaluation options
 * @param {boolean} options.strict - If true, throw on errors instead of returning 0
 */
function evaluateEquation(equation, state, rates, options = {}) {
  try {
    ...
    if (options.strict) {
      throw new Error(`Equation error in "${equation}": ${err.message}`);
    }
 ...
 * @param {number} width - Maximum width in characters (default: 40)
 * @returns {string} ASCII sparkline or empty string for empty/invalid data
 */
