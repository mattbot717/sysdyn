...
import { WEATHER_API, createWeatherDay } from './config.js';
...
export async function getForecast(days = 14) {
  ...
  for (let i = 1; i <= days && todayIndex + i < archive.all.time.length; i++) {
    ...
    forecastDays.push(createWeatherDay({
      date: archive.all.time[idx],
      precipitation_sum: archive.all.precipitation_sum[idx],
      temperature_2m_max: archive.all.temperature_2m_max[idx],
      temperature_2m_min: archive.all.temperature_2m_min[idx],
      et0_fao_evapotranspiration: archive.all.et0_fao_evapotranspiration[idx],
    }));
  }
 ...
 *
 * NOTE: Currently unused but kept for future CLI command:
 *   sys weather history [days]
 *
 * @param {number} days - Number of historical days to retrieve
 * @returns {Promise<Array>} Array of historical day objects with { date, rain_mm, temp_max_f, temp_min_f, et_mm }
 */
export async function getHistorical(days = 90) {
  ...
  for (let i = startIdx; i <= todayIndex; i++) {
    historicalDays.push(createWeatherDay({
      date: archive.all.time[i],
      precipitation_sum: archive.all.precipitation_sum[i],
      temperature_2m_max: archive.all.temperature_2m_max[i],
      temperature_2m_min: archive.all.temperature_2m_min[i],
      et0_fao_evapotranspiration: archive.all.et0_fao_evapotranspiration[i],
    }));
  }
