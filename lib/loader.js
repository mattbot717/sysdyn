...
export function parseYAML(yamlString) {
  ...
  for (let lineNum = 0; lineNum < lines.length; lineNum++) {
    ...
    if (!valueStr.startsWith('"') && !valueStr.startsWith("'")) {
      ...
      if (hashIndex !== -1) {
        valueStr = valueStr.substring(0, hashIndex).trim();
      }
    }

    if (valueStr === '|' || valueStr === '>') {
      // YAML block scalar: collect indented lines as a multi-line string.
      // '|' = literal (preserve newlines), '>' = folded (join with spaces).
      const fold = valueStr === '>';
      const blockLines = [];
      const blockIndent = indent + 2; // Expected indent of block content

      // Peek ahead to find the block content
      let nextLine = lineNum + 1;
      while (nextLine < lines.length) {
        const nl = lines[nextLine];
        ...
        if (nlIndent >= blockIndent) {
          blockLines.push(nl.slice(blockIndent));
          nextLine++;
        } else {
      ...
      if (fold) {
        parent[key] = blockLines.join(' ').trim();
      } else {
        parent[key] = blockLines.join('\n').trim();
      }

      // Advance line counter past the block content
      lineNum = nextLine - 1;
    } else if (valueStr === '') {
      // Nested object (no | or > indicator)
      const newObj = {};
      parent[key] = newObj;
