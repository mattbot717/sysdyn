...
<body>
  ...
  <script>
    ...
    function updateStatusCards(results) {
      const paddocks = [
        { id: 'cce', name: 'Cedar Crest East' },
        { id: 'ccw', name: 'Cedar Crest West' },
        { id: 'big', name: 'Big Pasture' },
        { id: 'hog', name: 'Hog Pasture' },
        { id: 'south', name: 'South Pasture' }
      ];
      ...
      paddocks.forEach(paddock => {
        ...
        if (current < 500) {
          status = 'critical';
          statusText = 'Critical - Hay Needed';
          cardClass = 'critical';
        } else if (current < 1000) {
          status = 'warning';
          statusText = 'Low - Monitor Closely';
          cardClass = 'warning';
    ...
    function updateRecommendations(results) {
      ...
      // Critical paddocks
      const critical = statuses.filter(s => s.forage < 500);
      ...
      // Low paddocks
      const low = statuses.filter(s => s.forage >= 500 && s.forage < 1000);
      if (low.length > 0) {
    ...
    async function runSimulation() {
      ...
      modelData.params.total_herd_weight = parseInt(herd) * 557; // Avg weight
